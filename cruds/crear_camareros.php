...
if (empty($_POST['nombre']) || empty($_POST['apellido']) || empty($_POST['telefono']) || empty($_POST['correo']) || empty($_POST['passwd'])   ) {
    echo "vacio";
...
if (count($resultado)==0) {
    ...
    echo "repetido";
        ...
        $query = $pdo->prepare("UPDATE tbl_usuarios SET usuario_nombre = :nombre, Apellido = :apellido,telefono=:telefono, correo=:correo,usuario_password:pass WHERE id = :id");
        $query->bindParam(":nombre", $nombre);
        $query->bindParam(":apellido", $apellido);
        $query->bindParam(":telefono", $telefono);
        $query->bindParam(":correo", $correo);
