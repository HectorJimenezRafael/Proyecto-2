...
$sql="SELECT * FROM `tbl_reserva_online` ro INNER JOIN tbl_horas_final h on ro.id_hora_res_o_final=h.id WHERE h.hora_final=CURRENT_TIME AND ro.dia=:hoy; ";
...
if (count($hay_reserva)>=1) {
        ...
        foreach ($hay_reserva as $mesa) {
             ...
             $sql="UPDATE tbl_recurso SET id_estado=1 WHERE id=:id";
             $query=$pdo->prepare($sql);
             ...
             $sql2="UPDATE tbl_reserva SET precio = 35, data_fin = current_timestamp(),finalizado = 1 WHERE id_recurso =:mesa   and finalizado = 0;";
             $query=$pdo->prepare($sql2);
             $query->bindParam(":mesa",  $id_mesa);
            //  $query->bindParam(":ocupacion",  $ocupacion);
             $query->execute();
